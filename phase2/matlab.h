...
public:
	...
	double calculate_rExpression(string operands[], int n, string operators_e[], int r,int& flag);
    CMatrix calculate_MExpression(string operands[], int n, string operators_e[], int r);
	void Extract(string s1, int pos, string& result, int& new_pos ,int& flag);
	void special_Extract(string s1, string& result,int& flag);

	string  getstring_arr(string** arr,int row,int col); 
	string** SetArray( string inputLine,int& rows,int& cols,int& flaag);
